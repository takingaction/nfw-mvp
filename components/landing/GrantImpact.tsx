      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-[#2d1239]/70 max-w-3xl mx-auto">
            Our microgrants aren't just numbers—they're lifelines that solve
            real problems.
          </p>
        </div>
          ...
          {impacts.map((impact, index) => {
            ...
            return (
              <div key={index} className="group relative overflow-hidden">
                <div
                  className={`${impact.bgColor} rounded-3xl p-8 lg:p-10 h-full transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl`}
                >
                  {/* Icon */}
                  <div className="w-20 h-20 bg-white/50 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300">
                    <IconComponent
                      className={`w-10 h-10 ${impact.iconColor}`}
                    />
                  </div>

                  {/* Content */}
                  <h3
                    className={`text-2xl lg:text-3xl font-black ${impact.iconColor} mb-3 font-bold`}
                  >
                    {impact.title}
                  </h3>
                  <p
                    className={`text-lg ${impact.iconColor}/80 leading-relaxed`}
                  >
                    {impact.description}
                  </p>
