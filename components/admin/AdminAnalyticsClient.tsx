...
import { Download, FileText, Users, DollarSign, Gift, TrendingUp } from 'lucide-react'
...
const COLORS = ['#2d1239', '#d4f1ad', '#b2d1ee', '#fdf493', '#bcafcf', '#4a1f5c']
...
export default function AdminAnalyticsClient({
  ...
  const membersByState = useMemo(() => {
    const map: Record<string, number> = {}
    profiles.forEach(p => { if (p.state) map[p.state] = (map[p.state] || 0) + 1 })
    return Object.entries(map)
      .sort(([, a], [, b]) => b - a)
      .slice(0, 10)
  ...
  const filteredGrants = useMemo(() =>
    grants.filter(g => g.submitted_at && new Date(g.submitted_at) >= cutoff),
    [grants, days]
  )
  ...
  const grantsByCategory = useMemo(() => {
    ...
    return Object.entries(map)
      .sort(([, a], [, b]) => b - a)
      .map(([cat, count]) => ({ category: GRANT_CATEGORY_LABELS[cat] || cat, count }))
  }, [filteredGrants])

  const totalFunded = useMemo(() =>
    grants.filter(g => g.status === 'funded').reduce((sum, g) => sum + (g.payout_amount || 0), 0),
    [grants]
  )

  const approvalRate = useMemo(() => {
    const submitted = filteredGrants.filter(g => g.status !== 'draft').length
    const approved = filteredGrants.filter(g => ['approved', 'funded'].includes(g.status || '')).length
    return submitted > 0 ? Math.round((approved / submitted) * 100) : 0
  }, [filteredGrants])

  // ── PERKS ────────────────────────────────────────────────
  const filteredRedemptions = useMemo(() =>
    redemptions.filter(r => r.created_at && new Date(r.created_at) >= cutoff),
    [redemptions, days]
  )
  ...
  const topOffers = useMemo(() => {
    ...
    return Object.entries(map)
      .sort(([, a], [, b]) => b - a)
      .slice(0, 8)
      .map(([offer, count]) => ({ offer: offer.length > 30 ? offer.slice(0, 30) + '…' : offer, count }))
  }, [filteredRedemptions])
  ...
  const statCards = tab === 'members' ? [
    { label: 'New Members', value: filteredProfiles.length, icon: Users, color: 'bg-[#2d1239]', text: 'text-white' },
    { label: 'Total Members', value: profiles.length, icon: Users, color: 'bg-[#bcafcf]/30', text: 'text-[#2d1239]' },
    { label: 'Active Paid', value: profiles.filter(p => p.subscription_status === 'active').length, icon: TrendingUp, color: 'bg-[#d4f1ad]', text: 'text-[#2d1239]' },
    { label: 'Est. MRR', value: `$${estimatedMRR}`, icon: DollarSign, color: 'bg-[#fdf493]', text: 'text-[#2d1239]' },
  ] : tab === 'grants' ? [
    { label: 'Applications', value: filteredGrants.length, icon: FileText, color: 'bg-[#2d1239]', text: 'text-white' },
    { label: 'Approval Rate', value: `${approvalRate}%`, icon: TrendingUp, color: 'bg-[#d4f1ad]', text: 'text-[#2d1239]' },
    { label: 'Total Funded', value: `$${totalFunded.toLocaleString()}`, icon: DollarSign, color: 'bg-[#fdf493]', text: 'text-[#2d1239]' },
    { label: 'Funded Grants', value: grants.filter(g => g.status === 'funded').length, icon: Gift, color: 'bg-[#b2d1ee]', text: 'text-[#2d1239]' },
  ] : [
    { label: 'Redemptions', value: filteredRedemptions.length, icon: Gift, color: 'bg-[#2d1239]', text: 'text-white' },
    { label: 'Total All Time', value: redemptions.length, icon: Gift, color: 'bg-[#bcafcf]/30', text: 'text-[#2d1239]' },
    { label: 'Unique Offers', value: new Set(filteredRedemptions.map(r => r.offer_key)).size, icon: TrendingUp, color: 'bg-[#d4f1ad]', text: 'text-[#2d1239]' },
    { label: 'Redeem Types', value: new Set(filteredRedemptions.map(r => r.redeem_type)).size, icon: FileText, color: 'bg-[#fdf493]', text: 'text-[#2d1239]' },
  ]
    ...
    <div>
          ...
          {(['members', 'grants', 'perks'] as const).map(t => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-semibold capitalize transition-colors ${
                tab === t ? 'bg-[#2d1239] text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'
              }`}
            >
              {t}
            </button>
      ...
      <div ref={dashboardRef}>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {statCards.map(card => (
            <div key={card.label} className={`${card.color} rounded-2xl p-5`}>
              <div className={`text-2xl font-black mb-1 ${card.text}`}>{card.value}</div>
              <div className={`text-xs font-semibold ${card.text} opacity-70`}>{card.label}</div>
            </div>
          ))}
        </div>
        ...
        {tab === 'members' && (
          <div className="space-y-6">
            <div className="bg-white rounded-2xl border border-gray-200 p-6">
              <h3 className="font-black text-[#2d1239] mb-4">New Members Over Time</h3>
              {membersByDay.length === 0 ? (
                <p className="text-gray-400 text-sm text-center py-8">No new members in this period.</p>
              ) : (
                <ResponsiveContainer width="100%" height={260}>
                  <LineChart data={membersByDay}>
                    ...
                    <Tooltip />
                    <Line type="monotone" dataKey="count" stroke="#2d1239" strokeWidth={2} dot={false} name="New Members" />
                  </LineChart>
                </ResponsiveContainer>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-6">
                <h3 className="font-black text-[#2d1239] mb-4">Top 10 States</h3>
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={membersByState} layout="vertical">
                    ...
                    <Tooltip />
                    <Bar dataKey="count" fill="#2d1239" radius={[0, 4, 4, 0]} name="Members" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
        ...
        {tab === 'grants' && (
          <div className="space-y-6">
            <div className="bg-white rounded-2xl border border-gray-200 p-6">
              <h3 className="font-black text-[#2d1239] mb-4">Grant Applications Over Time</h3>
              {grantsByDay.length === 0 ? (
                <p className="text-gray-400 text-sm text-center py-8">No applications in this period.</p>
              ) : (
                <ResponsiveContainer width="100%" height={260}>
                  <LineChart data={grantsByDay}>
                    ...
                    <YAxis tick={{ fontSize: 11 }} />
                    <Tooltip />
                    <Line type="monotone" dataKey="count" stroke="#2d1239" strokeWidth={2} dot={false} name="Applications" />
                  </LineChart>
                </ResponsiveContainer>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-6">
                <h3 className="font-black text-[#2d1239] mb-4">Applications by Category</h3>
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={grantsByCategory} layout="vertical">
                    ...
                    <Tooltip />
                    <Bar dataKey="count" fill="#d4f1ad" radius={[0, 4, 4, 0]} name="Applications" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
        ...
        {tab === 'perks' && (
          <div className="space-y-6">
            <div className="bg-white rounded-2xl border border-gray-200 p-6">
              <h3 className="font-black text-[#2d1239] mb-4">Redemptions Over Time</h3>
              {redemptionsByDay.length === 0 ? (
                <p className="text-gray-400 text-sm text-center py-8">No redemptions in this period.</p>
              ) : (
                <ResponsiveContainer width="100%" height={260}>
                  <LineChart data={redemptionsByDay}>
                    ...
                    <YAxis tick={{ fontSize: 11 }} />
                    <Tooltip />
                    <Line type="monotone" dataKey="count" stroke="#2d1239" strokeWidth={2} dot={false} name="Redemptions" />
                  </LineChart>
                </ResponsiveContainer>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-2xl border border-gray-200 p-6">
                <h3 className="font-black text-[#2d1239] mb-4">Top Offers Redeemed</h3>
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={topOffers} layout="vertical">
                    <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                    <XAxis type="number" tick={{ fontSize: 11 }} />
                    <YAxis dataKey="offer" type="category" tick={{ fontSize: 10 }} width={120} />
                    <Tooltip />
                    <Bar dataKey="count" fill="#b2d1ee" radius={[0, 4, 4, 0]} name="Redemptions" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
