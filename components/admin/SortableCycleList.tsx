...
export default function SortableCycleList({ cycles, grants }: Props) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const orderedCycleIds = orderedCycles.map((c) => c.id);
  ...
  return (
    <>
      {hasChanges && (
        <div className="fixed bottom-6 right-6 bg-white border border-nfw-blackberry shadow-lg p-4 flex items-center gap-4 z-50">
          <p className="text-sm text-nfw-blackberry font-ui">
            Unsaved changes
          </p>
          <button
            onClick={handleCancel}
            disabled={saving}
            className="flex items-center gap-1 px-3 py-2 text-sm font-ui text-nfw-blackberry hover:bg-nfw-blackberry/5 disabled:opacity-50"
          >
            <X className="w-4 h-4" />
            Cancel
          </button>
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex items-center gap-1 px-4 py-2 bg-nfw-blackberry text-white text-sm font-ui font-bold hover:bg-nfw-blackberry/90 disabled:opacity-50"
          >
