...
export default function DeleteCycleButton({
  ...
  const handleDelete = async () => {
    if (applicationCount > 0) {
      alert(
        `Cannot delete "${cycleName}" — it has ${applicationCount} application${applicationCount !== 1 ? "s" : ""}. Remove all applications first.`,
      );
      return;
    }
    ...
    if (
      !confirm(
        `Are you sure you want to permanently delete "${cycleName}"? This cannot be undone.`,
      )
    )
      return;
  ...
  return (
    <button
      ...
      title={
        applicationCount > 0
          ? `Cannot delete — ${applicationCount} application${applicationCount !== 1 ? "s" : ""} exist`
          : "Delete cycle"
      }
