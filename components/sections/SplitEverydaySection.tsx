...
export default function SplitEverydaySection({ content }: Props) {
  const c = content as SplitEverydayContent;
      ...
      <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-12">
        ...
        >
          ...
          <div className="space-y-7">
            {c.eyebrow && (
              <p className="font-ui text-xs font-black tracking-[0.06em] uppercase text-white">
                {c.eyebrow}
              </p>
            )}
            <h2 className="font-serif text-4xl lg:text-7xl text-nfw-dove leading-[1.1]">
              {parts[0]}
              <em className="italic">{c.headline_italic_phrase}</em>
              {parts[1]}
            </h2>
            <p className="font-serif text-2xl text-white">{c.body}</p>
            {c.cta_label && (
              <Link
                href={c.cta_url}
