...
### Session 2026-06-17: Fix Password Reset Mobile "Auth Session Missing"

#### Problem

Mobile users got "Auth Session Missing" error on password reset. Desktop worked fine.

#### Root Cause

Supabase's PKCE flow sends `?code=` in reset email links. The page was rendering the form immediately WITHOUT exchanging the code first. Without the code exchange, no session cookie was set, so the API call failed.

#### Solution

Updated `components/update-password-form.tsx` to:

1. On mount, extract `code` from URL search params
2. Call `supabase.auth.exchangeCodeForSession(code)` to establish session
3. Show "Verifying your reset link..." while exchange happens
4. Use `useRef` to prevent React StrictMode double-fire
5. Use `window.history.replaceState` to clean spent code from URL
6. Only render form AFTER session is confirmed ready
7. Handle errors gracefully (expired/used links)
