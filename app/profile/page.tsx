...
export default async function ProfilePage() {
  ...
  const membershipDisplay: Record<string, { label: string; color: string }> = {
    ...
    contributing: { label: 'Contributing Member', color: 'bg-[#b2d1ee]/30 text-[#2d1239]' },
    founding: { label: 'Founding Member', color: 'bg-[#BCAFCF]/30 text-[#2d1239]' },
  }
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-xl border border-[#2d1239]/10 p-6 mb-6">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-[#2d1239] mb-2">Membership Status</h3>
              ...
              {subscriptionStatus === 'canceling' && subscriptionEndsAt && (
                <p className="text-sm text-[#fdf493] mt-2 flex items-center gap-1">
                  <span>⚠️</span> Your membership will end on {subscriptionEndsAt.toLocaleDateString()}
                </p>
              )}
            </div>
        ...
        <div className="bg-white rounded-xl border border-[#2d1239]/10 p-6 mb-6">
          <h3 className="text-lg font-semibold text-[#2d1239] mb-4">Profile Information</h3>
          ...
          {profile ? (
            <div className="space-y-4">
              <div className="flex justify-between items-start py-2 border-b border-[#2d1239]/5">
                ...
                <p className="font-medium text-[#2d1239]">{profile.full_name || 'Not set'}</p>
              </div>
              <div className="flex justify-between items-start py-2 border-b border-[#2d1239]/5">
