...
export default async function MyApplicationsPage() {
  ...
  const statusCounts = {
    ...
    not_approved: grants?.filter(g => g.status === 'not_approved').length || 0,
    payment_pending: grants?.filter(g => g.status === 'payment_pending').length || 0,
    payment_sent: grants?.filter(g => g.status === 'payment_sent').length || 0,
  }
    ...
    <main className="min-h-screen bg-white">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[#2d1239]/60">Track your microgrant applications and their status</p>
            </div>
            <Link
              href="/grants/apply"
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
          {[
            { label: 'Submitted', value: statusCounts.submitted, color: 'text-blue-700' },
            { label: 'In Review', value: statusCounts.in_review, color: 'text-yellow-700' },
            { label: 'Approved', value: statusCounts.approved, color: 'text-green-600' },
            { label: 'Not Approved', value: statusCounts.not_approved, color: 'text-red-500' },
            { label: 'Pmt Pending', value: statusCounts.payment_pending, color: 'text-orange-600' },
            { label: 'Pmt Sent', value: statusCounts.payment_sent, color: 'text-purple-600' },
          ].map(stat => (
            <div key={stat.label} className="bg-[#f8f7fa] rounded-xl p-4 border border-[#2d1239]/5">
        ...
        {grants && grants.length > 0 ? (
          <div className="space-y-4">
            {grants.map((grant) => (
              <div key={grant.id} className="bg-white rounded-2xl border border-[#2d1239]/10 p-6 hover:shadow-lg transition-all duration-300">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2 flex-wrap">
                      <h3 className="text-lg font-black text-[#2d1239]" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                        {grant.grant_cycles?.cycle_name || 'Grant Application'}
                      </h3>
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold ${statusColors[grant.status] || 'bg-gray-100 text-gray-600'}`}>
                    ...
                    <p className="text-sm text-[#2d1239]/50">
                      Deadline: {grant.grant_cycles?.end_date
                        ? new Date(grant.grant_cycles.end_date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })
                        : '—'}
                    </p>
                  </div>
                  <div className="text-right flex-shrink-0 ml-4">
                    {grant.amount_approved ? (
                      <>
                        <p className="text-xl font-black text-[#2d1239]" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                          ${grant.amount_approved.toLocaleString()}
                        </p>
                        <p className="text-xs text-[#2d1239]/50">Approved amount</p>
                      </>
                      ...
                      <>
                        ...
                        <p className="text-xs text-[#2d1239]/50">Grant amount</p>
                      </>
                    ) : null}
                ...
                {grant.who_are_you && (
                  <p className="text-sm text-[#2d1239]/60 mb-4 line-clamp-2">{grant.who_are_you}</p>
                )}
                ...
                {grant.status === 'approved' && !grant.stripe_connect_account_id && (
                  <div className="bg-[#d4f1ad]/20 border border-[#d4f1ad] rounded-xl p-3 mb-4">
                    <p className="text-sm font-semibold text-[#2d1239]">🎉 Approved! Connect your bank account to receive your funds.</p>
                  </div>
                )}
                {grant.status === 'payment_pending' && (
                  <div className="bg-[#fdf493]/20 border border-[#fdf493] rounded-xl p-3 mb-4">
                    <p className="text-sm font-semibold text-[#2d1239]">💸 Your payment is being processed — arriving within 1-3 business days.</p>
                  </div>
                )}
                {grant.status === 'payment_sent' && (
                  <div className="bg-[#d4f1ad]/20 border border-[#d4f1ad] rounded-xl p-3 mb-4">
                    <p className="text-sm font-semibold text-[#2d1239]">✅ Payment sent! Check your bank account.</p>
                  </div>
                )}
          ...
          <div className="bg-[#f8f7fa] rounded-2xl border border-[#2d1239]/10 p-12 text-center">
            ...
            <h3 className="text-xl font-black text-[#2d1239] mb-2" style={{ fontFamily: 'Montserrat, sans-serif' }}>No Applications Yet</h3>
