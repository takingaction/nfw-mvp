...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Generate CSV
    const csvHeader = "email,name,amount,interval,subscription_id,customer_id,current_period_start\n";
    ...
    const csvRows = subscriptions.map(sub => {
      ...
      return `${email},${name},${amount},${interval},${subscription_id},${customer_id},${current_period_start}`;
    }).join("\n");

    const csv = csvHeader + csvRows;

    const date = new Date().toISOString().split('T')[0];
    const filename = `unmatched-subscribers-${date}.csv`;
    ...
    console.error("[unmatched-subscribers-export] Error:", error);
