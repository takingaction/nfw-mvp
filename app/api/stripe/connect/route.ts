...
export async function POST(request: NextRequest) {
  try {
    ...
    let accountId = grant.stripe_connect_account_id || profile?.stripe_connect_account_id
    ...
    if (!accountId) {
      ...
      await supabaseAdmin.from('grants').update({ stripe_connect_account_id: accountId }).eq('id', grantId)
    }
