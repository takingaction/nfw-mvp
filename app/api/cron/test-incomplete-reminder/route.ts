...
export async function GET() {
  return NextResponse.json({
    message: "TEST MODE - No emails will be sent. Use POST to actually send.",
    endpoint: "/api/cron/test-incomplete-reminder",
    method: "POST",
    instructions: "Send a POST request to this endpoint to test the incomplete member reminder.",
  });
}
...
export async function POST(request: Request) {
  ...
  if (!templateCheck.template) {
    console.log("[TEST-incomplete-member-reminder] Template not found, skipping");
    return NextResponse.json({ error: "Template not found" }, { status: 400 });
  }
  if (!templateCheck.isActive) {
    console.log("[TEST-incomplete-member-reminder] Template inactive, skipping");
  ...
  if (!contentCheck) {
    console.log("[TEST-incomplete-member-reminder] No published content, skipping");
  ...
  const hoursAgo = new Date(Date.now() - HOURS_THRESHOLD * 60 * 60 * 1000).toISOString();
  ...
  const { data: allProfiles, error } = await supabaseAdmin
    ...
    .lt("joined_at", hoursAgo)
    .limit(1000);

  if (error) {
    console.error("[TEST-incomplete-member-reminder] Error fetching profiles:", error);
  ...
  for (let i = 0; i < membersToProcess.length; i++) {
    ...
    try {
      ...
      if (emailResult.success) {
        ...
        console.error(`[TEST-incomplete-member-reminder] Failed to send to ${member.email}:`, emailResult.error);
        result.failed++;
      }
    } catch (err) {
      console.error(`[TEST-incomplete-member-reminder] Error sending to ${member.email}:`, err);
      result.failed++;
    }
  ...
  console.log(`[TEST-incomplete-member-reminder] Sent ${result.sent} emails, ${result.failed} failed`);
  return NextResponse.json({
    message: `Sent ${result.sent} emails, ${result.failed} failed`,
    eligibleCount: eligibleMembers.length,
    processedCount: membersToProcess.length,
    sent: result.sent,
