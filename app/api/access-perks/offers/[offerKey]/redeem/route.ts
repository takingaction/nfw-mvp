...
export async function POST(request: Request, { params }: RouteParams) {
  try {
    ...
    console.log('🔄 Redeeming offer:', { offerKey, method, memberKey, location_key }) // ✅ Log location_key
    ...
    console.log('📡 Calling redemption URL:', apiUrl.replace(process.env.ACCESS_OFFERS_TOKEN!, 'HIDDEN'))
    ...
    console.log('✅ Redemption API response:', JSON.stringify(redemptionData, null, 2))
    ...
    const usageRedeemKey = redemptionData.usage_redeem_key || 
    ...
    const promotionCode = redemptionData.promotion_code ||
    ...
    if (!phoneNumber) {
      const messageText = redemptionData.instructions || redemptionData.details?.display || redemptionData.display_message || ''
    ...
    const instructions = redemptionData.instructions || 
                        redemptionData.details?.display || 
                        redemptionData.display_message ||
                        redemptionData.message ||
                        null
                        
    const displayMessage = redemptionData.display_message ||
                          redemptionData.details?.display || 
                          redemptionData.message ||
                          null
    ...
    const { error: dbError } = await supabase
      ...
      .insert({
        ...
        location_name: offerDetails.physical_location?.location_name || 
                      offerDetails.physical_location?.city_locality || null,
