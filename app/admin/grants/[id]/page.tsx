...
export default async function AdminGrantCyclePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const grantsWithDocs = grants?.map(g => ({
    ...g,
    ...
    <main className="min-h-screen p-8 bg-gray-50">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            <h1 className="text-4xl font-bold text-[#2d1239] mb-2">{cycle.cycle_name}</h1>
            <p className="text-gray-600">
              {new Date(cycle.start_date).toLocaleDateString()} — {new Date(cycle.end_date).toLocaleDateString()} •{' '}
              ${cycle.amount_per_grant?.toLocaleString()} per grant • {cycle.grants_available} available
            </p>
          </div>
