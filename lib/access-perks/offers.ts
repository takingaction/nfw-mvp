...
export async function searchOffers(params: OfferSearchParams) {
  try {
    ...
    console.log('🌐 Full URL:', fullUrl.replace(process.env.ACCESS_OFFERS_TOKEN, 'HIDDEN_TOKEN'))
    ...
    if (!response.ok) {
      ...
      throw new Error(`Offers API Error: ${response.status} ${response.statusText} - ${responseText.substring(0, 200)}`)
    }
...
export async function getOffer(offerKey: string, memberKey: string) {
  try {
    ...
    console.log('Fetching offer from:', url.replace(process.env.ACCESS_OFFERS_TOKEN, 'HIDDEN_TOKEN'))
    ...
    const responseText = await response.text()
    console.log('Offer response (first 500 chars):', responseText.substring(0, 500))
 ...
 */
export async function getOfferUsesRemaining(offerKey: string, memberKey: string) {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Uses Remaining API Error: ${error.message || response.statusText}`)
    }
...
export async function getCategories(memberKey: string) {
  try {
    ...
    console.log('Fetching categories from:', url.replace(process.env.ACCESS_OFFERS_TOKEN!, 'HIDDEN_TOKEN'))
    ...
    const responseText = await response.text()
    console.log('Categories response (first 500 chars):', responseText.substring(0, 500))

    if (!response.ok) {
      throw new Error(`Categories API Error: ${response.status} ${response.statusText} - ${responseText.substring(0, 200)}`)
    }
